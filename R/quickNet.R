...
#' quickNet(mtcars)
#'
quickNet <- function(data, layout = 'spring', pie = TRUE, maximum = 0.47, groups = NULL, color = c("#71d0f5", "#fed439", "#66bb6a", "#fd7446", "#d2af81", "#d5e4a2", "#f44336", "#197ec0", "#46732e", "#8073ac", "#709ae1"), legend = FALSE, labels = NULL, nodeNames = NULL, border.width=2, border.color='#555555', ...){

  network <- estimateNetwork(data,default = "EBICglasso")

  if (!is.null(labels)) {
    colnames(network$graph) <- labels
    rownames(network$graph) <- labels
  }
  ...
  if (pie) {
    network_G$mgm_R2 <- mean(pred_mgm$error$R2, na.rm = T)
  }
