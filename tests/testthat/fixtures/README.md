...
`mtd-reference.json` was generated by executing Yuan and Shou's unmodified
S1 Code (`doi:10.1371/journal.pbio.3002758.s009`) with the absolute mean
derivative-product statistic. It records all shifts, statistics, exceedance
counts and bounds for seven cases. Source hashes and numerical conventions
are stored in the JSON. Regenerate with
`python tools/generate-mtd-reference.py --tts-source /path/to/test_independence.py`.
Python is not needed to run the stored R regression tests.

# Historical serialized fits

`legacy-29a414b.rds` and `legacy-72666a3.rds` contain complete fits created by
the original historical constructors in separate R processes, their historical
refit references and dependency manifests. They are generated from synthetic
data, without deleting fields to imitate old objects. See
[`docs/legacy-object-validation.md`](../../../docs/legacy-object-validation.md)
for source commits, scope and reproduction commands.
